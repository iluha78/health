...
По умолчанию используется драйвер `log`, который не отправляет письма, а сохраняет их содержимое в файл `backend/storage/logs/mail.log` — это удобно для локальной разработки. Чтобы включить реальную отправку через функцию `mail`, установите переменную окружения:
